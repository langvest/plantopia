...
public class PlantopiaAdvancements {
    ...
    public static final RegistryObject<PlantopiaSimpleAdvancement> COLLECT_ALL_HERBS = registerAdvancement("collect_all_herbs", PlantopiaSimpleAdvancement::new, MetaProperties.of(MetaType.CHALLENGE).parent(ROOT).icon(PlantopiaItems.HERBS_ICON));
    public static final RegistryObject<PlantopiaSimpleAdvancement> COLLECT_ALL_MUSHROOMS = registerAdvancement("collect_all_mushrooms", PlantopiaSimpleAdvancement::new, MetaProperties.of(MetaType.CHALLENGE).parent(ROOT).icon(PlantopiaItems.MUSHROOMS_ICON));
    public static final RegistryObject<PlantopiaSimpleAdvancement> PLACE_HOGWEED = registerAdvancement("place_hogweed", PlantopiaSimpleAdvancement::new, MetaProperties.of(MetaType.CHILD).parent(COLLECT_ALL_HERBS).icon(PlantopiaBlocks.HOGWEED));
    ...
    public static final RegistryObject<PlantopiaSimpleAdvancement> OBTAIN_TANSY = registerAdvancement("obtain_tansy", PlantopiaSimpleAdvancement::new, MetaProperties.of(MetaType.CHILD).parent(COLLECT_ALL_HERBS).icon(PlantopiaBlocks.TANSY).hidden());

    public static <T extends PlantopiaAdvancement> RegistryObject<T> registerAdvancement(String name, @NotNull Function<ResourceLocation, T> factory, @NotNull MetaProperties metaProperties) {
        return registerAdvancement(plantopia(name), factory, metaProperties);
    }

    public static <T extends PlantopiaAdvancement> RegistryObject<T> registerAdvancement(ResourceLocation identifier, @NotNull Function<ResourceLocation, T> factory, @NotNull MetaProperties metaProperties) {
        PlantopiaMetaBuckets.ADVANCEMENT.associate(identifier, new PlantopiaAdvancementMeta(identifier, metaProperties));
        return PlantopiaRegistries.ADVANCEMENT.register(identifier, () -> factory.apply(identifier));
    }
}
