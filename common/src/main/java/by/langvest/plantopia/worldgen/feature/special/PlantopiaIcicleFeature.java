...
public class PlantopiaIcicleFeature extends Feature<PlantopiaIcicleConfiguration> {
    ...
    @Override
    public boolean place(FeaturePlaceContext<PlantopiaIcicleConfiguration> context) {
        ...
        int height = random.nextFloat() < config.chanceOfTallerIcicle() && PlantopiaIcicleUtils.isEmptyOrWater(level.getBlockState(origin.relative(direction))) ? 2 : 1;
        PlantopiaIcicleUtils.growIcicleOnIceIfPossible(level, origin, direction, height, false, random);
        return true;
    }

    private static Optional<Direction> getTipDirection(LevelAccessor level, BlockPos pos, RandomSource random) {
        boolean canPlaceAbove = PlantopiaIcicleUtils.isValidGround(level.getBlockState(pos.above()));
        boolean canPlaceBelow = PlantopiaIcicleUtils.isValidGround(level.getBlockState(pos.below()));
    ...
    private static void createPatchOfIceBlocks(LevelAccessor level, RandomSource random, BlockPos pos, PlantopiaIcicleConfiguration config) {
        PlantopiaIcicleUtils.placeIceBlockIfPossible(level, pos);
        ...
        for (var direction : Direction.Plane.HORIZONTAL) {
            ...
            PlantopiaIcicleUtils.placeIceBlockIfPossible(level, pos1);
            ...
            PlantopiaIcicleUtils.placeIceBlockIfPossible(level, pos2);
            ...
            PlantopiaIcicleUtils.placeIceBlockIfPossible(level, pos3);
        }
    }
}
