...
public class PlantopiaFluffyDandelionBlock extends FlowerBlock {
    ...
    public void spawnBatchedParticles(BlockState state, ServerLevel level, BlockPos pos) {
    ...
    protected void spawnSingleParticle(BlockState state, Level level, BlockPos pos) {
        var particlePos = getRandomParticlePos(state, level, pos);

        level.addParticle(PlantopiaParticleTypes.FLUFFY_DANDELION_SEED.get(), particlePos.x(), particlePos.y(), particlePos.z(), 0, 0, 0);
    }

    protected Vec3 getRandomParticlePos(BlockState state, Level level, BlockPos pos) {
