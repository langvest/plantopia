...
public class PlantopiaCompats {
    ...
    private static void generateAll() {
        PlantopiaMetaBuckets.BLOCK.forEach(blockMeta -> {
            ...
            if (blockMeta.isFlammable()) {
				registerFlammable(block, blockMeta.getEncouragement(), blockMeta.getFlammability());
			}

            if (blockMeta.isCompostable()) {
				registerCompostable(block, blockMeta.getCompostability());
			}
    ...
    public static void registerFloweringWaterlily(PlantopiaFloweringWaterlilyBlock waterlilyBlock) {
    ...
    public static void registerCompostable(ItemLike itemLike, float compostability) {
        var registryHelper = Plantopia.getPlatform().getRegistryHelper();
        registryHelper.registerCompostable(itemLike, compostability);
    }

    public static void registerBrewable(ItemLike ingredient, Potion result, Potion precursor) {
        var registryHelper = Plantopia.getPlatform().getRegistryHelper();
    ...
    public static final class Compostability {
        ...
        public static final float PARTIAL_PLANT = PLANT_1 * 0.75F;
    }
