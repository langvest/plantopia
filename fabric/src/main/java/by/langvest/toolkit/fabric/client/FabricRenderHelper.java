package by.langvest.toolkit.fabric.client;
...
public class FabricRenderHelper extends RenderHelper {
    public FabricRenderHelper(Platform platform) {
        super(platform);
    }
    ...
    public void renderBlockModel(BlockState state, BakedModel model, BlockAndTintGetter level, BlockPos pos, PoseStack poseStack, VertexConsumer vertexConsumer, boolean checkSides, RandomSource random, long seed, int packedOverlay, Optional<RenderType> renderType, @NotNull Optional<Boolean> ambientOcclusion) {
        ...
        if (ambientOcclusion.isEmpty()) {
            blockModelRenderer.tesselateBlock(level, model, state, pos, poseStack, vertexConsumer, checkSides, random, seed, packedOverlay);
        } else if (ambientOcclusion.get()) {
            blockModelRenderer.tesselateWithAO(level, model, state, pos, poseStack, vertexConsumer, checkSides, random, seed, packedOverlay);
        } else {
            blockModelRenderer.tesselateWithoutAO(level, model, state, pos, poseStack, vertexConsumer, checkSides, random, seed, packedOverlay);
        }
    }
}
