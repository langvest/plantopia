...
public class PlantopiaBlocks {
	...
	public static final RegistryObject<Block> INFESTED_DIRT = registerBlock("infested_dirt", () -> new PlantopiaInfestedDirtBlock(Properties.copy(Blocks.DIRT)), MetaProperties.copy(MetaType.DIRT).customModel().customDrop());
	...
	public static final RegistryObject<Block> HOGWEED = registerBlock("hogweed", () -> new PlantopiaHogweedBlock(Properties.of().mapColor(MapColor.PLANT).noCollission().instabreak().sound(SoundType.GRASS).ignitedByLava().pushReaction(PushReaction.DESTROY).strength(0.8F)), MetaProperties.copy(MetaType.PLANT).tripleHigh().doubleWide().customModel().compostable(Compostability.PLANT_3 * 1.5F));
