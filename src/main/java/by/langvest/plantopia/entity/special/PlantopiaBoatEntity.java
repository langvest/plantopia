...
public class PlantopiaBoatEntity extends Boat implements PlantopiaBoatLike {
    private static final EntityDataAccessor<Byte> DATA_ID_TYPE = SynchedEntityData.defineId(PlantopiaBoatEntity.class, EntityDataSerializers.BYTE);

    public PlantopiaBoatEntity(EntityType<? extends Boat> entityType, Level level) {
        super(entityType, level);
    }

    public PlantopiaBoatEntity(Level level, double x, double y, double z) {
        super(PlantopiaEntityTypes.BOAT.get(), level);
        this.setPos(x, y, z);
        this.xo = x;
        this.yo = y;
        this.zo = z;
    ...
    protected void readAdditionalSaveData(CompoundTag compound) {
        ...
        if (compound.contains("ModType", CompoundTag.TAG_STRING)) {
            var identifier = ResourceLocation.parse(compound.getString("ModType"));
